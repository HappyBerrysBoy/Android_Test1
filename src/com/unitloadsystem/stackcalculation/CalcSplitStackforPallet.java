...
//		this.length = length;
	...
	public StackEvalutorBean CalcSplitStackRule(String firstDir, String secondDir, int containerWidth, int containerLength, float width, float length){
		...
		if(containerWidth >= containerLength){
			...
			int iAvailCount = (int)(containerLength / width);
			...
			for(int cnt = 1; cnt < iAvailCount + 1; cnt++){
				...
				int iLengthCount = (int)(containerWidth / length);
				double dUnitShare = (length * width) / (containerWidth * containerLength);
				double palletShare = dUnitShare * cnt * iLengthCount;
				
				AddBoxVer(aPalletView, cnt, iLengthCount, firstDir, 0, 0, width, length);
				
				if(cnt * width + length <= containerLength && width <= containerWidth){
					int iRemainWidthCnt = (int) ((containerLength - width * cnt) / length);
					int iAvailLengthCnt = (int)(containerWidth / width);
					palletShare += dUnitShare * iRemainWidthCnt * iAvailLengthCnt;
					
					AddBoxVer(aPalletView, iRemainWidthCnt, iAvailLengthCnt, secondDir, width * cnt, 0, length, width);
					
					if(iAvailLengthCnt * width + length <= containerWidth){
						if(cnt * width + width <= containerLength){
							int iRemainLengthCnt = (int)((containerLength - width * cnt) / width);
							int iAvailWidthCnt = (int)((containerWidth - width * iAvailLengthCnt) / length);
							palletShare += dUnitShare * iRemainLengthCnt * iAvailWidthCnt;
							
							AddBoxVer(aPalletView, iRemainLengthCnt, iAvailWidthCnt, firstDir, width * cnt, width * iAvailLengthCnt, width, length);
						}
					}
				}else if(iLengthCount * length + width <= containerWidth && length <= containerLength){
					int iRemainWidthCnt = (int) (containerLength / length);
					int iAvailLengthCnt = (int)((containerWidth - length * iLengthCount) / width);
					palletShare += dUnitShare * iRemainWidthCnt * iAvailLengthCnt;
					
					AddBoxVer(aPalletView, iRemainWidthCnt, iAvailLengthCnt, secondDir, 0, length * iLengthCount, length, width);
				}
	...
	private void AddBoxVer(ArrayList<PalletViewBean> palletView, int row, int col, String dir, float startrow, float startcol, float addrow, float addcol){
		for(int i=0; i<row; i++){
			for(int j=0; j<col; j++){
				palletView.add(new PalletViewBean(dir, (int)(startcol + addcol * j), (int)(startrow + addrow * i)));
			}
		}
	}
